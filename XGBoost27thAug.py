...
print("Start processing with XGBoost Regressor:")
...
overall_r2_scores_xgb = []
overall_mse_scores_xgb = []
overall_rmse_scores_xgb = []
combined_results_df_xgb = pd.DataFrame()
...
for activity in activities:
    ...
    overall_r2_scores_xgb.append(r2_xgb)
    overall_mse_scores_xgb.append(mse_xgb)
    overall_rmse_scores_xgb.append(rmse_xgb)
    
    # Combine predicted vs actual values into a single DataFrame
    results_df_xgb = pd.DataFrame({'Actual': y_test, 'Predicted': y_pred_xgb, 'Activity': activity})
    combined_results_df_xgb = pd.concat([combined_results_df_xgb, results_df_xgb])
    ...
    plt.title(f'Feature Importance for {activity} - XGBoost', fontsize=17, fontweight='bold')
    plt.xlabel('Importance', fontsize=14, fontweight='bold')
    plt.ylabel('Feature', fontsize=14, fontweight='bold')
    plt.show()

# Display only the first 5 values for all activities together
print("\nFirst 5 Predicted vs Actual values for all activities combined (XGBoost):\n")
print(combined_results_df_xgb.head(5))

# Print the overall scores
print("\nOverall Scores for All Activities (XGBoost):")
print(f"Average R² Value: {sum(overall_r2_scores_xgb) / len(overall_r2_scores_xgb)}")
print(f"Average Mean Squared Error: {sum(overall_mse_scores_xgb) / len(overall_mse_scores_xgb)}")
print(f"Average Root Mean Squared Error: {sum(overall_rmse_scores_xgb) / len(overall_rmse_scores_xgb)}")

# Plotting the predicted vs actual values
plt.figure(figsize=(14, 10))
for activity in activities:
    plt.scatter(combined_results_df_xgb[combined_results_df_xgb['Activity'] == activity]['Actual'], combined_results_df_xgb[combined_results_df_xgb['Activity'] == activity]['Predicted'], label=activity, alpha=0.5)
plt.plot([combined_results_df_xgb['Actual'].min(), combined_results_df_xgb['Actual'].max()],
         [combined_results_df_xgb['Actual'].min(), combined_results_df_xgb['Actual'].max()], 'r--', lw=2)
...
def get_member_details_and_predict(unique_key):
    ...
    predictions = {}
    for activity, model in models_xgb.items():
